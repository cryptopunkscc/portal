...
func (a Apps) Install(ctx context.Context, reader io.ReadCloser) (app apps.App, err error) {
	defer reader.Close()
	...
	find, err := a.Find(ctx, tmpName)
	if err != nil {
		panic(err)
		return
	}
	app = apps.App{App_: find[0]}
...
func (a Apps) InstallSources(ctx context.Context, sources ...target.Source) <-chan apps.App {
	c := make(chan apps.App)
	go func() {
		for _, app := range a.Resolve.List(sources...) {
			_, err := a.Install(ctx, app.File())
			c <- apps.App{App_: app, Err: err}
		}
	}()
	return c
}

func (a Apps) InstallFromPath(ctx context.Context, path string) (out <-chan apps.App, err error) {
	log := plog.Get(ctx).Type(a)
	log.Println("installing from path", path)
	...
	go func() {
		...
		for _, bundle := range found {
			log.Println("install bundle", bundle.Package())
			_, err = a.Install(ctx, bundle.Package().File())
			c <- apps.App{App_: bundle, Err: err}
		}
	}()
	return
}
