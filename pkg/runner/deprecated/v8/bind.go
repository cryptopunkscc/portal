...
func Bind(iso *v8.Isolate, core bind2.Core) (template *v8.ObjectTemplate, err error) {
	template = v8.NewObjectTemplate(iso)
	a := adapter{core}
	if err = template.Set(bind2.Log, v8.NewFunctionTemplate(iso, a.Log)); err != nil {
		return
	}
	if err = template.Set(bind2.Sleep, v8.NewFunctionTemplate(iso, a.Sleep)); err != nil {
		return
	}
	if err = template.Set(bind2.ServiceRegister, v8.NewFunctionTemplate(iso, a.ServiceRegister)); err != nil {
		return
	}
	if err = template.Set(bind2.ServiceClose, v8.NewFunctionTemplate(iso, a.ServiceClose)); err != nil {
		return
	}
	if err = template.Set(bind2.ConnAccept, v8.NewFunctionTemplate(iso, a.ConnAccept)); err != nil {
		return
	}
	if err = template.Set(bind2.ConnClose, v8.NewFunctionTemplate(iso, a.ConnClose)); err != nil {
		return
	}
	if err = template.Set(bind2.ConnWrite, v8.NewFunctionTemplate(iso, a.ConnWrite)); err != nil {
		return
	}
	if err = template.Set(bind2.ConnRead, v8.NewFunctionTemplate(iso, a.ConnRead)); err != nil {
		return
	}
	if err = template.Set(bind2.Query, v8.NewFunctionTemplate(iso, a.Query)); err != nil {
		return
	}
	if err = template.Set(bind2.GetNodeInfo, v8.NewFunctionTemplate(iso, a.NodeInfo)); err != nil {
		return
	}
	if err = template.Set(bind2.ResolveId, v8.NewFunctionTemplate(iso, a.Resolve)); err != nil {
		return
	}
	return
}
