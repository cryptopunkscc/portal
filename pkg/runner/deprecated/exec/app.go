...
import (
	...
	apphost2 "github.com/cryptopunkscc/portal/pkg/apphost"
...
func (d AppRunner) RunApp(ctx context.Context, manifest manifest.App, path string, args ...string) (err error) {
	...
	tokens := apphost2.Tokens{Dir: d.Config.Tokens}
	t, err := tokens.Get(manifest.Package)
	if err != nil {
		return err
	}
	authToken := t.Token.String()

	log := plog.Get(ctx).Type(d).Set(&ctx)
	log.Printf("app run: %s, %v", path, args)

	cmd := exec.CommandContext(ctx, path, args...)
	cmd.Cancel = func() error { return cmd.Process.Signal(os.Interrupt) }
	cmd.Env = slices.Concat(os.Environ(), d.Env(), []string{apphost.AuthTokenEnv + "=" + authToken})
	...
	log.Printf("app finished: %s, %v", path, args)
	return
}
