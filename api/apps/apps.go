...
type Apps interface {
	...
	Install(ctx context.Context, reader io.ReadCloser) (App, error)
	InstallSources(ctx context.Context, sources ...target.Source) <-chan App
	InstallFromPath(ctx context.Context, path string) (<-chan App, error)
}
